...
async updateLeaveStatus(req, res) {
  try {
    ...
    // Safely coerce dates
    const start =
      updatedRequest.startDate instanceof Date
        ? updatedRequest.startDate
        : new Date(updatedRequest.startDate);

    const end =
      updatedRequest.endDate instanceof Date
        ? updatedRequest.endDate
        : new Date(updatedRequest.endDate);
    ...
    await prisma.notification.create({
      data: {
        ...
        message: `Your leave request from ${start.toDateString()} to ${end.toDateString()} was ${status}.`,
        type:
          status === 'Approved'
            ? 'SUCCESS'
            : status === 'Declined'
