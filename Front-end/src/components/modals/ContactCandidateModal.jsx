...
HR Team`,
  ...
  const sendEmail = async () => {
    ...
    try {
      console.log('Sending email to candidate:', candidate.id);
      console.log('Email data:', emailData);

      // ACTUALLY CALL THE BACKEND API
      const response = await fetch(`/api/candidates/${candidate.id}/contact`, {
        method: 'POST',
        headers: {
      ...
      // Email sent successfully - now handle status changes
      if (statusChange && statusChange !== candidate.status) {
        try {
          console.log(`Updating candidate status from ${candidate.status} to ${statusChange}`);
          
          // Update candidate status via API
          const statusResponse = await fetch(`/api/candidates/${candidate.id}/status`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (statusResponse.ok) {
            ...
            alert(`✅ Email sent successfully!\nCandidate status updated to "${statusChange}"`);
          } else {
            console.error('Status update failed:', statusResult);
            alert(`✅ Email sent successfully!\n⚠️ But failed to update candidate status: ${statusResult.message}`);
          }
        } catch (statusError) {
          console.error('Status update error:', statusError);
          alert(`✅ Email sent successfully!\n⚠️ But failed to update candidate status: ${statusError.message}`);
        }
      } else {
        alert('✅ Email sent successfully!');
      }
        ...
        <button
          ...
          style={{
            ...
            background: isLoading 
              ...
              : 'linear-gradient(135deg, #0C3D4A, #1a4f5e)',
            color: 'white',
            border: 'none',
            borderRadius: '8px',
            cursor: isLoading ? 'not-allowed' : 'pointer',
