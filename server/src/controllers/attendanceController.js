...
export const checkOut = async (req, res) => {
  try {
    ...
    //  Overtime after 1 minute (1/60 hours) for demo
    const overtime = workingHours > (1/60) ? workingHours - (1/60) : 0;
    
  
    let status = 'PRESENT';
    //  Half day if less than 30 seconds (1/120 hours)
    if (workingHours < (1/120)) {
      status = 'HALF_DAY';
    } else if (overtime > 0) {
      status = 'OVERTIME';
    ...
    // Early departure logic adjusted for demo
    if (now < standardEndTime && workingHours >= (1/120)) {
